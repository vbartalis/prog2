 ...
 * csatornákat #include <cmath>	// mert vonunk gyököt a szóráshoz: std::sqrt
 * #include <fstream>	// fájlból olvasunk, írunk majd
 */

/*
 * Az LZWBinFa osztályban absztraháljuk az LZW algoritmus bináris fa építését.
 * Az osztály definíciójába beágyazzuk a fa egy csomópontjának az absztrakt
 * jellemzését, ez lesz a beágyazott Csomopont osztály. Miért ágyazzuk be? Mert
 * külön nem szánunk neki szerepet, ezzel is jelezzük, hogy csak a fa részeként
 * számiolunk vele.
 */
...
public class LZWBinFa {
   ...
   * tartalmazó objektum, lásd majd a védett tagok között lent: Csomopont gyoker;
   * A fa viszont már pointer, mindig az épülő LZW-fánk azon csomópontjára mutat,
   * amit az input feldolgozása során az LZW algoritmus logikája diktál:
   ...
   * előadásban tisztáztuk, hogy a tartalmazott tagok, most "Csomopont gyoker"
   * konstruktora előbb lefut, mint a tagot tartalmazó LZWBinFa osztály
   * konstruktora, éppen a következő, azaz a fa=&gyoker OK.)
   */
  /*
  ...
  public void egyBitFeldolg(char b) {
    ...
    if (b == '0') {
       ...
       * elkészítjük, azaz páldányosítunk a '0' betű akt. parammal Csomopont *uj
       * = new Csomopont ('0'); // az aktuális csomópontnak, ahol állunk azt
       * üzenjük, hogy // jegyezze már be magának, hogy nullás gyereke mostantól
       * van // küldjük is Neki a gyerek címét: fa->ujNullasGyermek (uj); // és
       * visszaállunk a gyökérre (mert ezt diktálja az alg.) fa = &gyoker; } else
       * // ha van, arra rálépünk { // azaz a "fa" pointer már majd a szóban
       * forgó gyermekre mutat: fa = fa->nullasGyermek (); } }
       *
       */
